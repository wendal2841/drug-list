...
import {
    IMedicine,
    IGetMedicineRequest,
    IPostMedicineRequest,
    IPutMedicineRequest,
    IDeleteMedicineRequest,
} from './types';
...
export default function* apiMedicine(): SagaIterator {
    ...
    yield takeEvery(MEDICINE.PUT.REQUEST, editMedicine as IPutMedicineRequest);
...
export function* editMedicine({ payload }: IAction<IMedicine>): SagaIterator {
    const data: IPostCollection<IMedicine> = {
        type: MEDICINE,
        collection: COLLECTION_PATH,
        data: payload,
